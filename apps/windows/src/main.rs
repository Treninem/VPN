...
impl AmriApp {
    ...
    fn sidebar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.with_layout(Layout::bottom_up(Align::LEFT), |ui| {
            ui.add_space(8.0);
            ui.label(RichText::new("Local Intelligence Engine").size(12.0).color(Color32::from_gray(125)));
            ui.horizontal(|ui| {
                let (rect, _) = ui.allocate_exact_size(Vec2::splat(8.0), egui::Sense::hover());
                ui.painter().circle_filled(rect.center(), 4.0, Color32::from_rgb(70, 210, 130));
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ...
                        ui.label(RichText::new(description).size(12.0).color(Color32::from_gray(145)));
                    });
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ui.toggle_value(value, if *value { "Вкл" } else { "Выкл" });
                    });
                });
            });
    }
    ...
    fn metric_card(ui: &mut egui::Ui, title: &str, value: &str, subtitle: &str) {
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ...
                ui.label(RichText::new(title).size(12.0).color(Color32::from_gray(145)));
                ui.add_space(6.0);
                ui.label(RichText::new(value).size(26.0).strong());
                ui.add_space(2.0);
                ui.label(RichText::new(subtitle).size(12.0).color(Color32::from_gray(125)));
            });
    }

    fn home(&mut self, ui: &mut egui::Ui) {
        ui.heading(RichText::new("Главная").size(30.0));
        ui.label(RichText::new("Автоматический выбор лучшего маршрута для каждого соединения").color(Color32::from_gray(150)));
        ui.add_space(18.0);
        ...
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ui.label(
                            RichText::new(if self.connected { "Защита включена" } else { "Защита выключена" })
                                .size(24.0)
                                .strong(),
                        );
                        ui.add_space(4.0);
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        let label = if self.connected { "Отключить" } else { "Подключить" };
        ...
        ui.horizontal_wrapped(|ui| {
            Self::metric_card(ui, "Активные маршруты", if self.connected { "4" } else { "0" }, "из горячего пула");
            Self::metric_card(ui, "Средний ping", if self.connected { "31 ms" } else { "—" }, "по активным маршрутам");
            Self::metric_card(ui, "Jitter", if self.connected { "2.8 ms" } else { "—" }, "realtime score");
            Self::metric_card(ui, "RouteScore", if self.connected { "94" } else { "—" }, "лучший текущий маршрут");
        });
    ...
    fn routes(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(RichText::new("Здесь будет живой список: приложение/домен → выбранный узел → причина решения").color(Color32::from_gray(150)));
        ui.add_space(18.0);
            ...
            egui::Frame::new()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                            ui.label(RichText::new(format!("Score {score}")).color(Color32::from_gray(140)));
                ...
                .show(ui, |ui| {
                    ...
                    match self.page {
                        ...
                        Page::Settings => self.placeholder(
                            ui,
                            "Настройки",
                            "Системная интеграция, темы, DNS, запуск с Windows, обучение и приватность.",
                        ),
                    }
                });
        });
