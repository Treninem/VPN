...
fn verify_readiness(state: WindowsForwardingState) -> WindowsForwardingReadiness {
    ...
    WindowsForwardingReadiness {
        ...
        // tproxy_setup returned successfully before state became RUNNING, so both default-route
        // capture and the explicit server bypass are installed for this generation.
